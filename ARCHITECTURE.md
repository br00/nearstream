...
> **Status:** Phase 3 shipped + Tier-1 polish + privacy + marks/empty-state/manifesto. The instance is a real multi-tenant room: User primitive at `users-meta/`, per-tenant R2 namespace `users/{userId}/...`, hostname-aware proxy that rewrites custom domains (`alessandroborelli.it/library` → `/alessandro/library`) without the handle ever appearing in the URL bar. Per-entry visibility (`public | private`) is wired through every read path and the RSS feed. Profile marks: every non-host tenant picks one of ten parameterised "human-circle" animations during onboarding (`lib/profile-mark-variants.ts` + `app/_components/site/profile-mark.tsx` + `profile-mark-picker.tsx`), persisted as `User.profileMark`, editable in `/studio` Profile, rendered at the top of their tenant home. `/reader` carries the platform's own animated Nearstream mark (`nearstream-animated-mark.tsx`) at top center. `/studio` shows a one-paragraph welcome when the tenant has no Letter / Stream / Essays / Inventory / Sources. `/manifesto` reads `NEARSTREAM.md` from `process.cwd()` at request time and renders it through `marked`. Signed-in visitors to `nearstream.app/` redirect to `/reader` (or `/onboarding` if they haven't picked a handle). A one-shot `POST /api/admin/migrate-host` copies the host's pre-Phase-3 R2 keys into their tenant namespace.
