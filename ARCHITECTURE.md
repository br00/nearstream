...
> - **Slice 33 — user owns rendering (Slice A: foundation + reader modes).** The realization mid-multi-image-lab: we were spending hours picking The Right gallery design for everyone, but Nearstream's whole point is people own their site and their reader. The instance should ship strong primitives + a curated set of *modes* per surface, and the user picks. Not "themeable everything" (Squarespace anti-pattern) — modes stay opinionated and finished, the vocabulary + protocol stay fixed. Foundation lands here: new `UserPreferences` open shape on `User` in `schemas/user.ts` (adding a surface later is one field, not a migration). New store method `setPreferences(id, patch)` in `lib/user-store.ts` merges a `Partial<UserPreferences>` so callers can update one surface without dropping others. New route `POST /api/preferences` validates each surface's value against its enum (`isReaderLayout`), supports both form-style submits and JSON. New "Display" section in `/settings` (id-anchored at `#display`) renders one fieldset per surface. First themed surface: `/reader`. Two modes — `"default"` (slice-29 app-density) and `"broadsheet"` (serif headlines via the system body face, mono kicker meta, italic picture captions, pulled-quote notes — V3 from the mobile lab brought to production). Picture rendering reuses `ReaderPicture` so the thumbnail / shimmer / cached-image fix from slice 30 applies to both modes. Lab pages from the round of design exploration are also in this PR — `app/design/multi-image-lab/page.tsx` (no-index) carries the V11–V16 rationale that led to "ship modes" instead of "ship one gallery." Future surfaces (tenant home, gallery, stream, letter) get their own slices and add to the picker.
