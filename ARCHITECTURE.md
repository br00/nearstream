...
> **Status:** Phase 2 · Slice 12 (Site identity — first site template, photo-led home) — `alessandroborelli.it` stops looking like generic Nearstream chrome and gets its own visual language. New `app/_components/site/` directory holds **user-site-template** components (distinct from `app/_components/` which is Nearstream chrome — see "Design system" below). First template component: `HumanCircle`, a ported moving.points generative piece that animates a machine attempting to draw a circle and adding human errors. Home page reshape: hero with the signature canvas + name + tagline, then recent photos grid (inventory), then essays list, then a compact stream feed. Stream is no longer the focal point of `/` — photos are.
...
**Site template layer** lives in `app/_components/site/` (introduced in slice 12). These are **user-specific** components — currently Alessandro's (a port of his moving.points generative work), but the directory shape anticipates Phase 3 multi-tenancy where each user picks a template that lives here or in a sibling. Site components inherit the Nearstream palette (`--background`, `--foreground`, etc.) so they compose naturally with chrome components but express a personal signature on top.
...
- **Chrome layer ≠ site-template layer.** `app/_components/` holds **Nearstream chrome** — components shared by every instance (NearstreamLockup, PageShell, Kicker, TagChip, etc.). `app/_components/site/` holds the **user's site template** — components specific to a particular user's expression. The chrome stays austere and recognizable across all instances; the site template carries personality. Both share the global token palette (`globals.css`) so they compose without color clashes. Slice 12 adds the first site-template component, `HumanCircle`. Phase 3+ may move per-user templates into per-user directories or a registry, but the chrome/site split is established now.
- **`HumanCircle` runs in canvas with `requestAnimationFrame`, fades old strokes via a translucent overlay.** Trade for the build-up effect: instead of `clearRect`-ing each frame, we `fillRect` a `rgba(0,0,0, 0.025)` over the canvas, then draw the new segment. Old strokes decay slowly; new strokes lay on top. After many cycles the canvas develops a denser texture — many overlapping circle-attempts — which is the moving.points aesthetic emerging from the algorithm itself rather than from a static asset. `prefers-reduced-motion` is honored: when set, a single static attempt is drawn instead of animating. `ResizeObserver` handles container resizes and resets the canvas backing store using `devicePixelRatio` for crisp rendering on retina. The component is a `"use client"` boundary; the page hosting it stays server-rendered around it.
...
| 12 (this) | Site identity (first site template) | new `app/_components/site/` directory + `HumanCircle` canvas component (ported from Alessandro's moving.points piece — machine traces a circle with human-feeling noise errors, accumulated with a fade trail), `app/page.tsx` reshape: hero canvas + name + tagline, recent photos grid, essays list, compact stream. ARCHITECTURE.md documents the chrome-vs-site layer split. |
