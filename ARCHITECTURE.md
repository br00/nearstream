...
> **Status:** Phase 2 · Slice 10 (Inventory in RSS) — Phase 2's publishing layer is complete. `/rss.xml` now merges all three primitives (Stream, Essay, Inventory) into one chronological feed. Inventory items ship with an `<enclosure>` (the full image — RSS-spec-correct way to attach media) and a `<description>` CDATA containing an `<img>` tag, the optional markdown description rendered to HTML, and a small metadata definition list (when dimensions / materials / edition / status / price are filled). Friends subscribed to your domain see everything you publish — notes, essays, and photographs — in one feed.
...
| 10 (this) | Inventory in RSS | `app/rss.xml/route.ts` pulls all three stores in parallel, renders inventory items with `<enclosure>` (full image) + `<description>` containing `<img>` tag, markdown description, and a `<dl>` of optional metadata fields |
