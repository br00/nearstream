...
> **Status:** Phase 3 · Slice 17 (Type detection + Nearstream RSS extension) — `FeedEntry.type` now lands as `note` / `essay` / `picture` instead of `unknown`. Our own `/rss.xml` advertises a `xmlns:nearstream="https://nearstream.app/ns/v1"` namespace and emits `<nearstream:type>` on every item so Nearstream-to-Nearstream feeds round-trip cleanly. Non-Nearstream feeds fall through a small heuristic (image enclosure → picture; title + ≥320-char body → essay; else note). Categories named `Stream`/`Essay`/`Inventory` are also recognized (for older Nearstream feeds without the extension).
