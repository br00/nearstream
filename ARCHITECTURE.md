...
> - **Slice 39 — voice notes.** First differentiator ship. Onboarded friends weren't posting — text notes have no on-ramp friction advantage over WhatsApp — so voice becomes the ripest content type to try. Pipeline: `AudioRecorder` client component wraps MediaRecorder (webm/opus on Chromium+Android, mp4/aac on Safari, native format preserved end-to-end); tap-to-start / tap-to-stop UX with 60s hard cap + live countdown + retake. Recorded blob PUTs directly to R2 via a new presigned endpoint `POST /api/stream/upload-url` (mirrors the inventory upload path but single-blob, no thumb) using the extended `media-store.ts` `ALLOWED_TYPES` (adds `audio/webm` + `audio/mp4`; new `isAllowedAudioType` gate keeps images and audio in their own endpoints). `POST /api/stream` gains `audioKey/audioMime/audioDurationMs` fields — text becomes optional when audio is present, so a caption-less voice note is a valid post; server rejects durations > `VOICE_NOTE_MAX_MS` (60_000ms cap in `schemas/stream.ts`). `StreamEntry.audio?: {key, mime, durationMs}` on the schema; `R2Store.delete` cascade-purges the audio blob when the entry is deleted (matches inventory's media cascade — media failure logged but non-fatal). New shared `AudioPlayer` client component wraps the `<audio>` element with a Web Audio `AnalyserNode`; RMS amplitude is written into a `MutableRef` each frame (not React state — 60Hz setState would rerender for no visible gain) and feeds the extended `AnimatedMark` from `human-circle.tsx`, which modulates `radiusRangeFrac` + `nMax` proportional to amplitude so the same Perlin visual "breathes" with the sound. Idle marks (no ref, `current === 0`) look identical to before — this is a non-breaking addition. AudioContext creates lazily on first user gesture (browsers reject fresh contexts outside a click). Voice chip added to `studio-composer.tsx` with its own client form (upload flow: presign → PUT → POST metadata; SubmitButton gains a `disabled` prop so "Post" stays off until a recording exists). Tenant home + `/stream` list render the player inline for voice entries; new detail route `app/[handle]/voice/[id]/page.tsx` gives voice notes a shareable URL with per-post metadata + `opengraph-image.tsx` — the OG PNG paints a static circle-of-points visualization (fixed seed since ImageResponse can't run our client canvas code) + author + duration + play glyph, the LinkedIn-share-ready surface. RSS emits `<nearstream:audio url mime durationMs>` + `<enclosure type="audio/webm|mp4">` + `<nearstream:type>voice</nearstream:type>` + inline `<audio controls>` in the description; RSS item link points at the detail URL for voice entries and the fragment link for plain notes. `lib/feed-parser.ts` gets `pickFeedAudio()` (Nearstream extension first, audio-enclosure fallback for arbitrary feeds — podcast episodes land in the `voice` bucket which is fine at MVP), `FeedEntryType` gains `voice`, `heuristicType` treats "has audio, no explicit type" as voice. Reader default + broadsheet feeds each get a voice card (player + optional caption). Digest gains `DigestEntryType = "voice"`; the row shows `▶ 0:23 · voice note` kicker + a mono ▶ glyph in the thumbnail slot. New env: none. Shared audio layer (`AudioPlayer` + audio-reactive `AnimatedMark` + `media-store` audio extension + `<nearstream:audio>` extension) is what slice 40 (music tracks as a Library primitive) consumes without re-implementing.
> - **Slice 38 — daily email digest.** The reader lives at a URL friends have to remember to open — with a small closed group posting sporadically, that's a hard habit to keep. Once-a-day digest fixes it: if any of a user's friends posted in the last ~24h, they get one email summarising the activity with per-item author + title + excerpt + optional thumb, and an "Open reader →" CTA. Cron endpoint at `app/api/cron/daily-digest/route.ts` — Vercel Cron pings `GET /api/cron/daily-digest` daily at 08:00 UTC (schedule in `vercel.json`), sending `Authorization: Bearer $CRON_SECRET`; the handler refuses anything else. Per user: skip if `preferences.emailDigest === "off"`, skip if `lastDigestAt` is within `MIN_INTERVAL_MS` (20h — an accidental double-fire within a day is a no-op but a legitimate next-day run passes), fetch sources + entries, build the digest via new `lib/digest.ts` (`buildUserDigest(entries, sources, since, until)` returns null on a quiet day), send via new `sendDigestEmail` in `lib/email.ts`, then update `lastDigestAt` via new `userStore.setLastDigestAt`. Quiet days still bump `lastDigestAt` so tomorrow's window is 24h, not 48h — otherwise two days of activity would collapse into one over-stuffed digest. Digest HTML template shares the `renderEmail` shell (mono footer, generous spacing) but per-item rows show a `AUTHOR · TYPE` kicker, title + excerpt, optional 96×96 thumbnail for pictures, and one shared "Open reader →" button at the bottom; text fallback body is plain scannable. `schemas/user.ts` gains `EmailDigestFrequency = "daily" | "off"` (default = `daily` — friends who join Nearstream are explicitly asking to hear from other friends; opt-out matches intent) and `User.lastDigestAt?: string` for the dedupe cursor. Settings `/settings#display` gets a matching fieldset alongside Reader layout / Inventory gallery / Site privacy so users can flip themselves to `off`. `POST /api/preferences` accepts the new field via the same validate-or-error pattern as the other surfaces. Idempotency contract: we'd rather skip a day than double-send, so `lastDigestAt` is updated after each successful send even when there was no activity.
> - **Slice 37 — onboarding + welcome-email polish.** The first-friend flow was rough: approving an access request sent the same "your sign-in link" email as a routine login (no context they'd been welcomed), and the post-onboarding save dropped them straight into `/studio` with no moment for their URL. Split: `lib/email.ts` now exports `sendWelcomeMagicLink` alongside `sendMagicLink`, sharing an HTML shell via a `renderEmail` helper but with distinct subject ("You're in — welcome to Nearstream"), lead ("{host} approved your request. Welcome."), and button label ("Get started"). The approve route (`app/api/access-requests/[id]/approve/route.ts`) uses the welcome variant. The onboarding save endpoint redirects to `/welcome` instead of `/studio`. New page `app/welcome/page.tsx` renders the new friend's ProfileMark at 200px, a "You're in, {firstName}" heading, their absolute site URL with a ShareUrlButton, and two numbered next-steps: "Add a friend → /reader/friends" and "Write something → /studio". Signed-out visitors get redirected to `/login`; users without a handle bounce to `/onboarding`; existing users can revisit `/welcome` anytime as a first-steps reference. Reserved handle set gets `welcome` + `request-access` so nobody claims either as a URL slug.
> - **Slice 36 — LinkedIn-ship privacy + access requests.** Prep for going public: friends' tenants can't be exposed to strangers guessing handles. Adds `User.preferences.sitePrivacy: "public" | "friends" | "private"` with `lib/tenant-visibility.ts` as the single enforcement point on every tenant read path (home, stream, library index + detail, inventory index + detail, essay detail, RSS). Defaults via `defaultSitePrivacy(email)` — host stays `public` so the LinkedIn-linked tenant works; everyone else defaults to `friends` so a random URL guess doesn't reveal their site. Non-owner + private/friends without a session → redirect to `/login?next=...&reason=private-tenant` so the visitor can auth as themselves; non-owner + private with a session → `notFound()` (don't leak existence). New `/request-access` public form + `POST /api/access-requests` + admin queue in `/settings#host` under Host tools with approve/deny buttons. Approve mints an `AllowlistEntry` in a new persisted `allowlistStore` (extends the env `ALLOWED_EMAILS` seed) and sends a welcome magic link; deny is silent. `isEmailAllowed` in `lib/auth.ts` is now async and consults both env + persisted store; `isEmailAllowedSync` retained for callers that only need the seed. RSS handles the internal-vs-external distinction via a shared secret header (`NEARSTREAM_INTERNAL_SECRET`) — same-instance reader fetches send it and bypass the visibility gate so friends' feeds still flow to friends' readers. Reader-side 404 handling now purges the cached FeedEntry rows for that source so a friend flipping to `private` immediately drops out of friends' feeds instead of leaving stale content behind. Landing page CTA gets a "Request an invite" affordance; login page shows a matching link on both the form and the `sent=1` acknowledgement. New `app/robots.ts` disallows authed surfaces + `/request-access`; tenant pages stay technically crawlable so `public` sites can be linked but rely on their existing `noindex` meta for search-index protection.
> - **Slice 34 — multi-image reaches the reader.** Slice 33 shipped multi-image inventory + a per-tenant gallery on the detail page, but the *reader* card still showed only the cover for friends' multi-image posts — the RSS extension only carried the cover thumbnail (`<nearstream:thumbnail>`), so the parser dropped the rest. New extension `<nearstream:image url width height thumbUrl thumbWidth thumbHeight />` emitted once per image in `app/[handle]/rss.xml/route.ts`, with `<enclosure>` kept alongside for non-Nearstream readers. Parser in `lib/feed-parser.ts` gains `pickRssImages` which prefers the new extension and falls back to multiple `<enclosure>` / `<media:content>` tags for arbitrary feeds. `FeedEntry` schema gains `images?: FeedEntryImage[]` alongside the legacy `image?`; `feedImagesOf(entry)` is the canonical read path. Reader picture cards (both `default` and `broadsheet` modes) now render the cover full-width with a `· N` count badge bottom-right and a horizontal-scroll filmstrip of up to 8 additional thumbs underneath (`+N` indicator beyond 8). Tap anywhere on the card still goes to the friend's tenant page — the reader is for "see what they're up to," the tenant page is for the full viewing experience. Existing stored entries get the new `images` field populated on the next refresh because slice 30's `upsertMany` always overwrites.
> - **Slice 33 — user owns rendering + multi-image inventory.** The realization mid-multi-image-lab: we were spending hours picking The Right gallery design for everyone, but Nearstream's whole point is people own their site and their reader. The instance ships strong primitives + a curated set of *modes* per surface, and the user picks. Not "themeable everything" (Squarespace anti-pattern) — modes stay opinionated and finished; vocabulary + protocol stay fixed. **Foundation:** new open-shape `UserPreferences` on `User` in `schemas/user.ts`; `setPreferences(id, patch)` on `lib/user-store.ts` merges so callers can update one surface without dropping others; `POST /api/preferences` validates each field against its enum; new `/settings#display` section renders one fieldset per surface. **Themed reader (first surface):** two modes — `"default"` (slice-29 app-density) and `"broadsheet"` (serif headlines, mono kicker meta `Mon, 26 Jun`, italic picture captions, pulled-quote notes — V3 from the mobile lab brought to production). Picture rendering reuses `ReaderPicture` so the slice-30 thumbnail / shimmer / cached-image fix applies to both modes. **Multi-image inventory:** `InventoryItem` gains `images: InventoryImage[]` alongside legacy `image?: InventoryImage` (mirrored from `images[0]` for one release). New read helper `imagesOf(item)` is the canonical access path. `app/_components/inventory-upload-form.tsx` rewritten for multi-file: per-tile preview with shimmer placeholder, per-tile remove, "cover" badge on `images[0]`, parallel upload of N originals + N thumbs with aggregated progress, 12-image cap mirrored server-side. `/api/inventory/upload-url` accepts `{ images: [{contentType}] }` and returns `{ uploads: [{upload, thumb}] }`; legacy single-image shape still works. `/api/inventory` validates the array; `validateImages` accepts new `images` or legacy `image`. Tenant home, library hub, inventory index all show `· N` count badges on covers via the new `imagesOf()` helper. RSS emits one `<enclosure>` per image plus a Nearstream `<nearstream:thumbnail>` for the cover; `<description>` body includes one `<img>` per image. **Themed gallery (second surface):** new `InventoryGallery` client component reads `User.preferences.galleryLayout`. `"contact-sheet"` (default) = V16 from `/design/multi-image-lab` — two-column tile grid, full-bleed modal viewer on tap with arrow keys + prev/next + body-scroll lock; adapts to a stack for `N=2` (V15 adaptive) and a single image for `N=1`. `"stack"` = quieter alternate — every image native-ratio, full-width, no modal. Lab pages from the design exploration (no-index) carry the V11–V16 rationale that led to "ship modes" instead of "ship one gallery."
...
| `CRON_SECRET` | bearer token the daily-digest cron handler requires (slice 38). Vercel Cron auto-includes it as `Authorization: Bearer $CRON_SECRET`. Missing → the handler returns 500 (fail closed). | `openssl rand -hex 32` |

**`vercel.json` — cron schedule only.** Kept minimal on purpose. The only entry is the daily-digest cron (slice 38); everything else uses Next.js defaults. When Fly is the target, remove `vercel.json` and either wire the cron there or move it to an external scheduler pinging `/api/cron/daily-digest`.
