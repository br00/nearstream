...
> **Status:** Phase 3 · Slice 18 (Reader home `/reader`) — the four-slice reader arc lands. A gated `/reader` route now renders the mixed chronological feed of every friend's entries with the same shape as the Gosia-tested prototype: small author + time stamp at the top of each entry, then the type-shaped body — notes are plain text, essays show a big title + preview behind a left-border, pictures lead with the image and a quiet caption. Empty states for "no sources yet" and "sources but nothing fetched" both bridge back to the studio refresh / source-add affordances. Authors link to their `siteUrl` (if set) or the entry URL; entries link to their permalink — both in a new tab, because the reader is the room you live in, not somewhere you bounce out of permanently.
