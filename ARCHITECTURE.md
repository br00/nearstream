...
> **Status:** Phase 3 shipped + Tier-1 polish + per-post privacy live on `alessandroborelli.it` and `nearstream.app`. The instance is a real shared room: friends sign in by email, pick a handle, get their own site at `nearstream.app/{handle}` (or their own custom domain when they have one). Tenant pages, RSS, and the reader all enforce a `public` / `private` visibility flag per post. The mid-2026 "stretch" goals from `NEARSTREAM.md §07` — multi-tenant + reader at minimum — are done. Next chunks live at the bottom of this doc under "What's next per slice"; the manifesto's Phase 6 (federation, replies, ActivityPub bridge, POSSE) is the long arc.
>
