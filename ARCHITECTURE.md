...
> **Status:** Phase 3 · Slice 19 (Multi-tenant) — the instance turns into a real shared room. New `User` primitive (`schemas/user.ts` + `lib/user-store.ts` at R2 prefix `users-meta/`). Auth session now carries `userId`; first sign-in finds-or-creates a user and routes through `/onboarding` to pick a handle + display name. All seven content stores (stream, essay, inventory, letter, source, feed-entry) now accept `userId` and namespace R2 at `users/{userId}/...`. Public routes moved from the root into `app/[handle]/...` (Notebook home, library, library/[slug], library/inventory, library/inventory/[slug], stream, rss.xml). A hostname-aware proxy reads a `TENANT_DOMAINS` env map and rewrites custom-domain requests internally (`alessandroborelli.it/library` → `/alessandro/library`) so URLs stay clean. `nearstream.app/` is now a tiny marketing landing for signed-out visitors and auto-redirects signed-in users to `/reader` or `/onboarding`. A one-shot `POST /api/admin/migrate-host` (host-only button in `/studio`) copies the host's legacy R2 keys into the new tenant namespace.
