...
- **Animated Nearstream mark at the top of /reader.** Same noise engine as Alessandro's signature, distinct parameters (tighter brush, denser stamps, faster morph), pinned at the top center of the reader. *Why: the reader is the only screen everyone shares; giving it a live mark — drawn by the same algorithm that draws the signatures below it — quietly tells the room "this is the same family of marks." Cheaper than a logo and harder to copy.* (2026-06-06)
- **Manifesto reads at /manifesto, sourced from NEARSTREAM.md at request time.** No build-time inlining, no separate doc site. *Why: NEARSTREAM.md is already the source of truth; mirroring it at /manifesto means doc edits ship with the next merge and the GitHub copy and the web copy can never disagree.* (2026-06-06)
