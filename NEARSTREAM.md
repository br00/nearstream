...
- **Wet ink is removed.** A freshly published entry no longer renders faint. *Why: it read as a rendering fault rather than a choice — reported as a bug twice in one afternoon, once on a friend's photograph (a dimmed photo looks like a failed image load) and once on a just-published essay (you publish something and then can't see it). It also applied to every viewer rather than only the author, which made no sense: the argument was about denying the author an instant reward, and a visitor is in no such loop. A mechanic that needs a caption explaining that it's deliberate has already failed. Age alone carries the same argument and does it quietly.

  Two rescues were considered and rejected. A **margin cue** — the entry at full strength with the date reading `just now` until it set — solves the "looks broken" problem but is then merely a timestamp, which we already have. **Delaying propagation** rather than display — normal to you, absent from RSS and the digest for a few minutes — is the more interesting version, and it fails for a different reason: a deliberate delay is indistinguishable from a slow system, exactly as dimming is indistinguishable from a failed load. Both borrow a signal that already means something is wrong.

  The deeper objection applies to every version: a delay only means anything if someone is waiting. Nobody is. And by the time this mechanic runs there are no likes, counts or notifications left — the loop it interrupts has already been dismantled, so it is belt-and-braces on a system that has no trousers.* (2026-09-02)
- **The tenant home is one dated document, not category sections.** Everything the viewer may see — Lines, voice, pictures, tracks, essays — merged into a single reverse-chronological timeline, with no pagination. *Why: the sectioned home read as four filing cabinets when full and as empty when sparse, and every entry rendered identically (a date, then text) so a photograph, a one-liner and an essay were the same object. Now the kind IS the form: a short Line is set at 32px because one sentence is the whole post, a long one drops to 15px in a narrow column, pictures go full width without a frame, audio is a band, an essay is the only thing with a rule and a headline. Library remains the by-subject half — Stream is the diary, Library is the commonplace book.* (2026-08-29)
- **Older entries fade; looking at one brings it back.** Opacity and a hair of blur derived from `publishedAt`, floored at 45% so nothing becomes unreadable, with hover/focus restoring fully. *Why: recency should come from optics, not from an algorithm deciding an order or a "load more" hiding the past. Nothing is ever removed from the page — it just goes quiet, the way a print left in the light does. Researched against Bear Blog, Mataroa and Micro.blog, none of which paginate or rank; Mataroa puts every post on one page and that's the rule adopted here.* (2026-08-29)
- **Fresh entries are still setting.** Anything under six hours old renders faint and darkens as it ages. *Why: you cannot see your own new post at full strength immediately, which is the inverse of the instant-feedback loop every other posting surface is built on. Needs a line of explanation somewhere or a friend reads it as a bug.* (2026-08-29)
- **The studio composer groups by territory: Stream / Library.** Two chips, Stream active on arrival with the Line composer already open, group members as a text sub-row. *Why: the flat row of primitives measured 537px against 327px of usable width at 375px, so three of six chips were visible and Essay + Letter sat behind a horizontal scroll on every phone — and it got worse with each primitive added. Grouping is fixed-width forever; a new Library type joins a group instead of extending a row. The groups aren't invented for the layout, they're the split this document already uses: Stream is the running feed that syndicates, Library is the things that keep a permanent URL. Stream open by default because the most casual primitive should cost zero taps — that's the surface friends aren't using.* (2026-08-27)
- **"Letter" is renamed "Now", and it leaves the composer.** Its own block on `/studio`, collapsed once written. *Why: it was never a post. The store is get/set rather than list/add; it has no id, slug or visibility; and it appears nowhere in RSS, the reader or the digest — it never leaves your own page. Sitting in "Post something" beside five things that genuinely syndicate made it read as a sixth way to publish. "Now" is the IndieWeb convention (the /now page) for exactly this: what you're up to at the moment, updated when it changes. "Letter" implied a newsletter, which §04 explicitly rejects. UI rename only — storage stays at `users/{id}/site/letter.json` and the route stays `/api/letter`, because renaming the key would orphan every existing record for a word.* (2026-08-27)
- **The short-text Stream primitive is called a "Line".** *Why: "note" implies length — people expect a paragraph. "Drop someone a line" already carries the brevity, and Line / Voice reads as two small human things rather than two file formats.* (2026-08-27)
...
- **Three authed surfaces, three URLs, one intent each.** `/studio` posts (Letter / Stream / Essay / Inventory), `/reader` reads + manages friends, `/settings` profile + export + sign-out. Friend-graph management lives at `/reader/friends` with a `Friends · N →` pill on the feed linking to it. Top nav across the three is always the two cross-links you're not on. *Why: Federico (first non-host tenant) said he "got a bit lost" in the previous monolith /studio — eight sections on one page with no clear "where do I start." Three URLs with one intent each maps onto a post / read / settings mental model that nobody needs explained. Sources pair with consumption (you read these people, you manage them in the same room) and settings becomes the tiny rarely-touched page it should be.* (2026-06-13)
- **"Friends" is the user-facing word, not "Following" or "Sources."** Pills, headings, buttons, empty-state copy all say Friends / friend. The codebase keeps `sourceStore` / `Source` internally because that's the technical primitive shape (an RSS source, which could in principle be non-friend), but the surface language matches the §01 lexicon entry "friend graph — like contacts on a phone." *Why: "Following" is straight Twitter / Instagram vocabulary and imports their subscribe-to-creators mental model. "Sources" is precise but CMS-y. "Friends" is honest about what this actually is and is what the manifesto already calls the concept.* (2026-06-13)
- **Stream tag is a mode, not a discipline.** Three broad buckets — `making` (writing, cooking, sketching, building…), `taking in` (reading, watching, listening…), `being` (feeling, noticing, idling, just life…). Default selection is `being`. Each picker row carries its hint inline so people don't have to guess what each one catches. *Why: discipline tags ("Code / Photo / Music / Writing") fit Alessandro and nobody else, and would silently turn into hashtags the moment a friend joined who knits or cooks. Modes describe the shape of a moment instead of the identity of its author — a knitter and a coder both fit "making" without inventing new tags. Three is enough structure to give the archive a tiny shape; few enough to never make anyone think hard.* (2026-06-11)
- **Profile marks instead of profile photos.** Every non-host tenant picks one of ten parameterised "human-circle" animations during onboarding; it sits where a profile photo would on a normal social product. *Why: a face is exactly the wrong primitive for a network whose point is to stop performing for strangers. The mark is a moving signature, not a likeness. Ten variants is enough to feel personal without becoming a customisation surface that has to be moderated.* (2026-06-06)
- **Manifesto reads at /manifesto as a structured page, not rendered markdown.** /manifesto is hand-laid in Tailwind + Nearstream chrome (numbered sections, lexicon grid, decisions-as-cards, anti-aspirations grid, build phases with shipped/doing/next/later badges). NEARSTREAM.md stays in the repo as the internal working doc (decisions log + raw thinking); the curated public view lives in `app/manifesto/page.tsx`. *Why: the markdown-rendered version was a wall of prose; the schematic page makes the load-bearing decisions actually readable. Trade-off: two surfaces to sync — accepted because NEARSTREAM.md is mostly the decisions log, which is the only section that changes often, and the cost of duplicating one line each time is small.* (2026-06-11, supersedes the 2026-06-06 entry that said /manifesto rendered NEARSTREAM.md verbatim)
- **Studio first-time empty state.** When a freshly-onboarded tenant opens /studio with no content of any kind, the page leads with a single-paragraph welcome pointing at "post a stream entry below" and showing the tenant's URL. *Why: removes the "blank dashboard with eight unfilled forms" first impression for people who don't know which form to touch first.* (2026-06-06)
...
- **Multi-tenant via hostname-aware proxy, not per-tenant subdomains.** Each user has a `handle` and a per-user R2 namespace (`users/{userId}/...`). The proxy reads the host header and a `TENANT_DOMAINS` env map (`alessandroborelli.it:alessandro,…`) and rewrites custom-domain requests internally to `/{handle}/...`. Friends who don't have a domain live at `nearstream.app/{handle}` until they buy one. *Why: keeps the "your domain is your identity" promise for friends who have one, gives a clean path-based fallback for friends who don't, and means one Vercel project / one R2 bucket / one DNS zone serves everyone. Per-tenant subdomains (`gosia.nearstream.app`) would force wildcard SSL + DNS automation we don't need at this size.* (2026-06-07)
- **Tenant content links are canonical from the start, not via redirect.** Every tenant-page link goes through a `tenantBase(handle)` helper that returns either an absolute custom-domain URL (`https://alessandroborelli.it/library`) or an instance path (`/gosia/library`). The proxy still strips `/{handle}/...` → `/...` on custom domains as a defensive fallback for stale bookmarks and pasted URLs, but the happy path produces clean URLs natively. *Why: the earlier "let the proxy redirect everything" version added a 302 round-trip + URL-bar shimmer on every internal click. Native canonical URLs cost nothing extra at render time and feel honest to the user.* (2026-06-09)
- **Per-post visibility is two-level: `public` / `private`.** Stored as an optional field on Stream, Essay, Inventory schemas (defaults to `public` on read for backward compat). Owners see all their content; non-owners see only `public`. Private detail pages `notFound()` (don't leak slug existence). RSS omits private entirely. Letter stays public — it's the host's broadcast slot, not per-entry content. *Why: shipping the two ends of the spectrum unlocks the "drafts and journal" use case immediately and answers most friend-asked "how do I post something private?" questions. A `friends` level needs a friend-graph primitive + instance-authenticated tenant rendering + RSS auth — real architecture, deferred until a real friend asks for it. The two-level slice is a small honest piece; the three-level extension is a planned slice in its own right.* (2026-06-10)
- **Edits preserve URLs.** Editing a Stream entry / Essay / Inventory item updates only the body fields — `id`, `slug`, `publishedAt`, and (for inventory) `image` stay frozen at first publish. The URL never changes, RSS guids stay stable across edits, and existing links keep working. The cost: you can edit the title of an essay but the slug shows the original title, which is mild dissonance. *Why: a typo fix shouldn't break a friend's bookmark or replay every entry in their RSS reader. Permanent-URLs-from-publish is a stronger contract than always-canonical-slugs. If a renamed essay's slug really matters, delete + repost is the documented escape hatch.* (2026-06-09)
- **Self-export is a ZIP, not just JSON.** "Download all my content" returns a zip containing `nearstream-export.json` + every inventory image as actual bytes under `media/{key}` + a `README.txt` describing the layout. *Why: the manifesto promises "ownership through exit" and URLs aren't ownership — if R2 ever goes away, JSON-only links break. The zip is a real self-contained snapshot. A future import endpoint on another Nearstream instance can read this format verbatim — the `format` field in the JSON points at a stable version URI.* (2026-06-09)
- **`nearstream.app` is the instance home, not a separate marketing project.** The old `nearstream-landingpage` Vercel project (which served `nearstream-khaki.vercel.app`) is retired; `nearstream.app` now points at the main `nearstream` project and the multi-tenant landing in `app/page.tsx`. The `/about` page ports the longer explanation from the old landing verbatim so the prose survives. *Why: keeping two projects for the same domain meant two sources of truth for "what does Nearstream say it is" and double maintenance. One project, one deploy, one landing.* (2026-06-09)
